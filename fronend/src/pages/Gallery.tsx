            ...
            {CATEGORIES.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`sans-ui pb-2 transition-all duration-300 relative group flex items-center gap-2 ${
                    activeCategory === cat ? 'text-royal-gold' : 'text-white/40 hover:text-white'
                  }`}
                >
                  <span className="text-sm tracking-[0.2em]">{cat}</span>
                  <span className="text-[10px] opacity-40 font-mono">({count})</span>
                  <span className={`absolute bottom-0 left-0 h-[1px] bg-royal-gold transition-all duration-500 ${
            ...
            {filteredItems.map((item, i) => (
              <motion.div
                ...
                className={`group relative overflow-hidden glass cursor-pointer ${
                  i % 4 === 1 || i % 4 === 2 ? 'md:row-span-2 md:h-auto' : ''
                }`}
